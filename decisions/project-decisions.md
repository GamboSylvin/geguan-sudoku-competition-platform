...
These rows answer questions Q1 to Q6 of `context/_stakeholder-question-pack.md` and the context builder's follow-ups. They were answered on 2026-09-26 through the project owner, who answered in the stakeholder's role, and are recorded as **Confirmed** (tag **[C]** in `requirements/REQUIREMENTS.md`). If one changes later, it is treated as the stakeholder changing their mind: a new dated row supersedes it. They apply to the **Individual stage** unless a row says otherwise. Where a row extends an earlier one, both are kept.
