...
- **Late manual submit (decided 2026-09-26).** A submit that arrives after the server timer ended is not counted as manual; the last saved grid is auto-submitted instead, with no grace period. This is for the Individual rounds; the team rotation is in `REQUIREMENTS.md` §4.1. **[T]** (SUB-003, SUB-006)
...
- **Answer check and unique solutions (I-14).** The answer is compared with the solution stored with the question. A puzzle with several valid solutions would reject a correct answer. Options: puzzles with a unique solution in the question file, or a check against the puzzle's rules. Not decided. **[O]**
