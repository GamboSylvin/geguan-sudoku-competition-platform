...
- **Session length and idle behavior (I-16).** How long a login lasts, and what happens when a session expires or a tablet sleeps, are not decided. Constraints: a session survives the whole event day and a page refresh, an expired session must not lose saved answers, and one active device per account applies (PAR-005). **[O]**
